...
function bob(text: string): string {
    text = text.trim()
    let response: string = '';
    let pattern = /[a-zA-Z]+/
 ...
 *      the: 2 
 * */ 
function contarPalabras(text: string): Map<string, number> {
...
class Squares {
    public count: number
    public sumSquares: number = 0
    public squareSum: number = 0
...
function gigaSecond(date: Date, seconds: number): Date {
...
function revertirString(text: string): string {
    let array_text: string[] = text.trim().split("")
    let reverse: string[] = []
...
class Triangle {
    ...
    private setNumbers: Set<number> = new Set()
